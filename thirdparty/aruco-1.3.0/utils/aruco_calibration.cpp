...
float_t ViewPointDistance = 0.025;
using namespace std;
...
aruco::CameraParameters getDummyInitialCameraParams(cv::Size imageSize) {
    ...
    CamMatrix.at< float_t >(0, 0) = 500;
    CamMatrix.at< float_t >(1, 1) = 500;
    CamMatrix.at< float_t >(0, 2) = imageSize.width / 2;
    CamMatrix.at< float_t >(1, 2) = imageSize.height / 2;
...
void setCurrentViewPoint(vector< cv::Point3f > &objPoints, vector< cv::Point2f > &imagePoints) {
    ...
    if (minDistanceToStoredLocations(Tvec) > ViewPointDistance) {
        ...
        if (TheobjPointsV.size() >= 3 && TheobjPointsV.size() < 8) {
            ...
            float_t repro = cv::calibrateCamera(TheobjPointsV, TheimagePointsV, TheCamParams.CamSize, TheCamParams.CameraMatrix, TheCamParams.Distorsion,
                                              The_rvecs, The_tvecs);
...
int32_t main(int32_t argc, char **argv) {
    ...
    try {
        ...
        while (TheVideoCapturer.grab() && key != 27) { //
            TheVideoCapturer.retrieve(TheInputImage);
            // detect
            float_t prob = TheBoardDetector.detect(TheInputImage);
        ...
        if (TheobjPointsV.size() >= 3) {
            ...
            int32_t nToRemove = 0, nTotal = 0;
            vector< vector< bool_t > > toRemove(TheobjPointsV.size());
